...
class Database(object):
	...
	def __get_hostname(self):
		...
		"""
		try:
			if utils.is_set(self.__DB_HOST):
	...
	def __get_username(self):
		...
		"""
		try:
			if utils.is_set(self.__DB_USER):
	...
	def __get_user_password(self):
		...
		"""
		try:
			if utils.is_set(self.__DB_PASSWORD):
	...
	def __get_db_name(self):
		...
		"""
		try:
			if utils.is_set(self.__DB_NAME):
	...
	def __convert_resultset_based_on_datatype(self, resultset, datatype=None):
		...
		"""
		try:
			if utils.is_set(resultset):
				return None

			if datatype == dict:
				return [result for result in resultset]
			elif datatype == list:
				return [result[0] for result in resultset]
			elif datatype == tuple:
				return tuple([result[0] for result in resultset])
			else:
				return tuple([result[0] for result in resultset])
		except Exception, e:
			raise e
